                ...
                <div id="content" style="padding:20px;">
                    <?php foreach ($package_paginate as $key => $value) : ?>
                        <div style="display:inline-block;background-color:rgba(55,55,55,1);width:40%;margin:10px;">
                            <div>
                                <p style="color:white;text-align:center;padding:10px 0;background-color:rgba(25,25,25,0.2)"><?= $value['name'] ?></p>
                            </div>
                            <div>
                                <div>
                                    <img style="width:10vw;" src="https://s3.amazonaws.com/preview.thegamecrafter.com/7C2E1DC8-245A-11EA-AA99-974C4E5FF538.png" class="w-3/5" />
                                </div>
                                <p style="color:white;background-color:rgba(105, 105, 105, 0.7);text-align:center;margin:10px;padding:10px;">
                                    <?= $value['donate'] ?> + <?= $value['bonus'] ?>% = <?= ceil($value['donate'] + ($value['donate'] * ($value['bonus'] / 100))) ?> donate
                                </p>
                                <?php if ($value['donate_bonus']) : ?>
                                    <div style="color:white;background-color:rgba(55, 75, 105, 0.7);text-align:center;padding: 10px;margin:10px;">
                                ...
                                <?php endif; ?>
                                <p style="color:white;margin-bottom: 20px;background-color:rgba(155,155,155,0.2);padding:10px;margin:0 10px;margin-bottom: 20px;">R$<?= $value['value'] ?></p>
                                <div style="color:White;background-color:rgba(105, 105, 105, 0.7);text-align:center;margin:10px;padding:20px 0;">
                                    <a style="color:white;color:white;background-color:rgba(105, 105, 155, 0.7);text-align:center;margin:10px;padding:10px;" href="<?= base_url('auth/purchasemp/' . $value['id']) ?>">
                                        Mercadopago
                                    </a>
                                </div>
                                <div style="color:White;background-color:rgba(105, 105, 105, 0.7);text-align:center;margin:10px;padding:20px 0;">
                                    <a style="color:white;background-color:rgba(105, 105, 155, 0.7);text-align:center;margin:10px;padding:10px;" href="<?= base_url('auth/purchasepic/' . $value['id']) ?>">
                                        Picpay
                                    </a>
                                </div>
                            </div>
